...
export default function MotorikArmLinksCard() {
  const [selectedValueArmLinks, setSelectedValueArmLinks] = useState(null)

  return (
    <div className="space-y-3">
      <h3 className="font-semibold">5b Motorik linker Arm</h3>

      <div className="flex flex-col gap-3">
        {armOptions.map((option) => {
          const isSelected = selectedValueArmLinks === option.value
          ...
          return (
            <button
              key={option.value}
              type="button"
              onClick={() => setSelectedValueArmLinks(option.value)}
